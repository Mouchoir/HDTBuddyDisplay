...
namespace HDTBuddyDisplay
{
    public class MoveCardManager
    {
        ...
        private void MouseInputOnMouseMoved(object sender, EventArgs eventArgs)
        {
            ...
            var pos = User32.GetMousePos();
            double mouseVerticalPositionAdjust = (yMouseDeltaFromCard * Settings.Default.BuddyCardScale / 100);
            double mouseHorizontalPositionAdjust = (xMouseDeltaFromCard * Settings.Default.BuddyCardScale / 100);
            var p = Core.OverlayCanvas.PointFromScreen(new Point(pos.X - mouseHorizontalPositionAdjust, pos.Y - mouseVerticalPositionAdjust));

            Settings.Default.BuddyCardTop = p.Y;
            Settings.Default.BuddyCardLeft = p.X;
        }
